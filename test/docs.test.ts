...
describe("runtime documentation", () => {
	it("documents idle-gated UI-only web_search final card semantics", async () => {
		...
		const requiredSnippets = [
			...
			"Flush immediately from `message_end` when the runtime is already idle",
			"filtered from LLM context",
			"persisted/replayed through UI-only custom entries",
	...
	it("documents web_search live overlay lifecycle, timing defaults, and image_generation exclusion", async () => {
		...
		const requiredSnippets = [
			...
			"collapse 3000 ms and hide 8000 ms",
			"only the final card delivery, q manual close, session lifecycle cleanup, runtime dispose, or a UI failure closes it",
			"At most one live overlay is open at a time",
