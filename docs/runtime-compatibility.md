...
| Image persistence locations | Observed | OMP 14.7.3 session manager exposes artifact directory, save, and allocation APIs; the plugin also supports configured output directories and an agent image directory. | Persist generated image files in the first available location: configured `output.directory`, session artifact directory, then the agent image directory under the runtime agent home. |
...
| Image-generation model opt-in metadata | Required for image generation | A broad provider match can cover both image-capable and non-image backend routes. The selected model must expose `compat.openaiProviderTools.imageGeneration: true` or equivalent legacy metadata. | Inject `image_generation` only when selected-model metadata opts in; leave `web_search` unaffected. |
...
Detect explicit runtime or capability metadata when available. The primary configuration source for provider-native tool eligibility is the runtime model metadata preserved from `models.yml`, especially `compat.openaiProviderTools`. Legacy `.omp/.pi/openai-provider-tools.yml` files remain supported in deterministic project-before-user order for existing installations; when runtime identity is unknown, the plugin warns visibly rather than silently ignoring one runtime family.
