...
| Image-generation model opt-in metadata | Required for image generation | A broad provider match can cover both image-capable and non-image backend routes. The selected model must expose `compat.extraBody.openai_provider_tools.image_generation: true`, equivalent capability metadata, or an equivalent header flag. | Inject `image_generation` only when both plugin config and model metadata opt in; leave `web_search` unaffected. |
...
- Keep all provider tools disabled unless the matching config explicitly sets `enabled: true`; additionally require selected-model opt-in metadata before enabling `image_generation`.
